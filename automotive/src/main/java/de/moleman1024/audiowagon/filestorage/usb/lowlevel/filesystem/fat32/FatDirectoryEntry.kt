 ...
 */ private constructor(
     ...
     */
    private var data: ByteBuffer = Util.allocateByteBuffer(SIZE)
) {
    ...
    var createdDateTime: Long
        ...
        get() = decodeDateTime(
            getUnsignedInt16(CREATED_DATE_OFF),
    ...
    var lastModifiedDateTime: Long
        get() = decodeDateTime(
            getUnsignedInt16(LAST_WRITE_DATE_OFF),
     ...
     * @see .setDirectory
     */
    constructor() : this(data = Util.allocateByteBuffer(SIZE)) {
    ...
    companion object {
        ...
        fun createVolumeLabel(volumeLabel: String): FatDirectoryEntry {
            ...
            val buffer = Util.allocateByteBuffer(SIZE)
            buffer.order(ByteOrder.LITTLE_ENDIAN)

            System.arraycopy(
                volumeLabel.toByteArray(Charset.forName("ASCII")), 0, buffer.array(), 0,
                volumeLabel.length
            )
         ...
         */
        fun createLfnPart(
            unicode: String, offset: Int, checksum: Byte,
            index: Int, isLast: Boolean
        ): FatDirectoryEntry {
