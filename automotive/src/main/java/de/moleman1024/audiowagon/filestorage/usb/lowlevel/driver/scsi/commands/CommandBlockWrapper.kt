...
package de.moleman1024.audiowagon.filestorage.usb.lowlevel.driver.scsi.commands
...
protected constructor(
    var dCbwDataTransferLength: Int,
    /**
     ...
     *
     * This method should be overridden in subclasses whose data length changes as data is received (dynamic
     * length messages). The function returns the new total data transfer length.
     *
     * @param buffer
     ...
     */
    open fun dynamicSizeFromPartialResponse(buffer: ByteBuffer): Int {
        throw NotImplementedError("If dynamic length possible override in subclass")
    }

    override fun toString(): String {
        val tempBuffer = ByteBuffer.allocate(64)
        serialize(tempBuffer)
        val data =
            tempBuffer.array().joinToString(separator = " ") { "%02x".format(it) }.substring(0, tempBuffer.position())
        return "CommandBlockWrapper(data=0x $data)"
    }
