...
/**
 * This class represents a long file name entry. The long file name can be accessed via [.getName]. This class
 * delegates most actions to the [.actualEntry]. It is responsible for parsing and serializing long file names and
 * the actual entry with the corresponding short name.
 *
 * To understand the structure of long file name entries it is advantageous to look at the official FAT32 specification.
 *
 * @author mjahnen
 */
...
private constructor(
    ...
    fun serialize(buffer: ByteBuffer) {
        lfnName?.let { lfnName ->
            ...
            var entry = FatDirectoryEntry.createLfnPart(
                lfnName, index * 13, checksum, index + 1, true
            )
            entry.serialize(buffer)

            while (index-- > 0) {
                entry = FatDirectoryEntry.createLfnPart(
                    lfnName, index * 13, checksum, index + 1, false
                )
                entry.serialize(buffer)
            }
        }
    ...
    companion object {
         ...
         */
        fun read(
            actualEntry: FatDirectoryEntry, lfnParts: List<FatDirectoryEntry>
