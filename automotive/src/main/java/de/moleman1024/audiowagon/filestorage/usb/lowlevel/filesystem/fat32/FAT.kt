...
import de.moleman1024.audiowagon.filestorage.usb.lowlevel.util.LRUCache
 ...
 * [#getBytesPerCluster()][Fat32BootSector]
...
internal constructor(
    private val blockDevice: BlockDeviceDriver, bootSector: Fat32BootSector,
    ...
    init {
        if (!bootSector.isFatMirrored) {
            ...
            logger.verbose(TAG, "FAT is not mirrored, fat $fatNumber is valid")
        } else {
            ...
            logger.verbose(TAG, "FAT is mirrored, fat count: $fatCount")
        }
    ...
    internal fun getChain(startCluster: Long): Array<Long> {
        ...
        // For performance reasons we always read or write two times the block size this is esp. good for long cluster
        // chains because it reduces of read or writes and mostly cluster chains are located consecutively in the FAT
        val buffer = Util.allocateByteBuffer(bufferSize)
    ...
    internal fun free(chain: Array<Long>, numberOfClusters: Int): Array<Long> {
        ...
        if (offsetInChain > 0) {
            ...
            // if we freed all clusters we have to write the last change of the for loop above
            buffer.clear()
            blockDevice.write(lastOffset, buffer)
        }
        logger.verbose(TAG, "freed $numberOfClusters clusters")
        ...
        fsInfoStructure.write()
        val arr = chain.copyOfRange(0, offsetInChain)
        if (arr.isNotEmpty()) {
            cache[arr[0]] = arr
        }
        return arr
